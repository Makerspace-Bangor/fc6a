...
COMMAND_DESCRIPTIONS = {
    ...
    "RA": "Read A operand data; address format and A-area meaning are not yet documented.",
            ...
            if operand in ("D", "M", "_"):
                ...
                if address is not None:
                    ...
                    if operand == "D":
                        ...
                        item_unit = "words"
                        end_addr = (
                            address + item_count - 1
                            if item_count else address
                        )
                        end = str(end_addr)
                        register_range = (
                        ...
                        register_range = (
                            f"T{address}" if end_addr == address
                            else f"T{address}-T{end_addr}"
                        )

            else:
                # Unknown/generic operand addresses may contain hexadecimal
                # characters or another command-specific encoding. Preserve
                # the four-character field exactly rather than forcing decimal.
                start = address_text
                item_count = count
                item_unit = "bytes"
                register_range = (
                    f"{operand}{address_text} ({count} bytes)"
                )
...
def reply_for_request(frame):
    ...
    # Read the byte/count field independently of the operand address.
    # Generic operand addresses such as RA289F0A may contain A-F.
    try:
    ...
    if command.startswith(b"R"):
        # Unknown read types receive the requested number of zero bytes.
        # Their address field is treated as command-specific opaque text.
